...
$(document).ready(function () {
    ...
    $('form#serialize').submit(async function (e) {
        ...
        console.log(data)
        console.log(sessionStorage.getItem(data.token))
        ...
        if (data.editorType) {
            ...
            console.log(Array.isArray(data.editorType))

            if (Array.isArray(data.editorType)) {
                data.editorType.map((e) => {
                    console.log(e)
                    let editorAA = $('#' + e + ' .ql-editor');
                    console.log('#' + e + ' .ql-editor')
                    data[e] = editorAA[0].innerHTML;
                })
            } else {

                let editorAA = $('#' + data.editorType + ' .ql-editor');
                data[data.editorType] = editorAA[0].innerHTML;
            ...
            /* const mesajElem = $('#' + data.editorMesajID + ' .ql-editor');tion
...
async function cartUSer(data, token) {
    ...
    form_data = new FormData();
    form_data.append("userID", data.userID);
    form_data.append("product", data.product);
    form_data.append('token', token);

    await $.ajax({
        url: 'api/sponsor/cart.php',
        cache: false,
        contentType: false,
        processData: false,
        async: true,
...
async function sortSave(data, token) {
    ...
    let url = "api/pages/sort.php";
    form_data = new FormData();

    form_data.append('token', token);
    form_data.append('list', data.list);
